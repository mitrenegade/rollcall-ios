...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    double percent = (double)_data.length / self.expectedLength;
}

- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    NSLog(@"Async Connection did finish loading");
...
{
    NSLog(@"Async Connection did fail");
    self.connection = nil;
    self.data = nil;
...
{  
        ...
        if (connection.URL == URL || [connection.URL isEqual:URL])
        {
            ...
            for (int j = i - 1; j >= 0; j--)
            {
                ...
                if (earlier.target == connection.target &&
                    earlier.success == connection.success && [earlier.URL isEqual:URL])
                {
                    [earlier cancel];
                    [_connections removeObjectAtIndex:j];
                    NSLog(@"Removing connection with URL %@", earlier.URL);
                    i--;
                }
            }
            
            //cancel connection (in case it's a duplicate)
            NSLog(@"Cancelling connection with URL %@", connection.URL);
            [connection cancel];
            
            //perform action
            UIImage *image = [notification.userInfo objectForKey:AsyncImageImageKey];
            NSURL *url = connection.URL;
            objc_msgSend(connection.target, connection.success, image, url);
    ...
    NSLog(@"Connections remaining: %d", [_connections count]);
    //update the queue
    [self updateQueue];
}
        ...
        if (success) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if ([target respondsToSelector:success])
                    [target performSelector:success withObject:image withObject:URL];

                NSDictionary *info = [NSDictionary dictionaryWithObject:image forKey:AsyncImageImageKey];
                [self notify:@"kAsyncImageLoadedFromCache" object:target userInfo:info];
            });
...
- (void)setImageURL:(NSURL *)imageURL
{
    super.imageURL = imageURL; // super is the category
...
-(void)updateProgress:(float)percent {
    //NSLog(@"Progress for %@: %f", self.imageURL, percent);
}
